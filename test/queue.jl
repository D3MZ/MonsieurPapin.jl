...
@testset "queue" begin
    entries = frontier(2, WET)
    drain!(entries, wet.([0.9, 0.4, 0.1, 0.3]))
    @test length(entries) == 2
    @test best!(entries).score == 0.1
    @test best!(entries).score == 0.3
    @test isnothing(best!(entries))

    source = Channel{WET}(4) do channel
        foreach(score -> put!(channel, wet(score)), [0.9, 0.4, 0.1, 0.3])
    end
